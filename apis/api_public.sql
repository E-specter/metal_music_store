...
BEGIN 
    ...
    IF p_correo !~ '^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+[.][A-Za-z]+$' THEN 
    ...
    -- Validar contraseña
    IF NOT public.validar_contrasena(p_contrasena) THEN
        RETURN jsonb_build_object(
            'error',
...
BEGIN 
    ...
    ORDER BY 
        CASE 
            WHEN p_orden = 'recientes' THEN p.fecha_creacion
            WHEN p_orden = 'precio_asc' THEN p.precio::timestamp
            WHEN p_orden = 'precio_desc' THEN (-p.precio)::timestamp
            ELSE p.fecha_creacion
        END DESC,
        CASE 
            WHEN p_orden = 'mejor_calificados' THEN (
                SELECT -COALESCE(AVG(calificacion), 0)
                FROM resenas_productos
                WHERE id_producto = p.id_producto
                AND esta_aprobado
...
BEGIN 
    ...
    SELECT COALESCE(AVG(calificacion), 0) INTO v_calificacion_promedio
    FROM resenas_productos
    WHERE id_producto = p_id_producto
    AND esta_aprobado;
...
BEGIN
    ...
    -- Crear promociones estáticas
    v_promociones := jsonb_build_array(
        jsonb_build_object(
            'id',
            1,
            'titulo',
        ...
        jsonb_agg(
            jsonb_build_object(
                ...
                jsonb_build_object(
                    ...
                    u.nombres,
                    'iniciales',
                    SUBSTRING(u.nombres FROM 1 FOR 1) || SUBSTRING(u.apellidos FROM 1 FOR 1)
                ),
                'calificacion',
                r.calificacion,
                'comentario',
...
ON productos(id_producto) WHERE esta_activo;

-- Para búsqueda geográfica
CREATE INDEX IF NOT EXISTS idx_ubicaciones_geo 
ON ubicaciones USING gist(coordenadas);

-- Para autenticación
CREATE INDEX IF NOT EXISTS idx_usuarios_auth 
ON usuarios(nombre_usuario, correo_electronico) WHERE esta_activo;
